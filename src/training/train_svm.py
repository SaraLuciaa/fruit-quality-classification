...
SVM Training Script
-------------------
Trains an SVM model for fruit quality classification.
Usage: python -m src.training.train_svm [--data_dir data] [--output_dir experiments]
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train SVM fruit quality classifier")
...
def plot_confusion_matrix(cm, output_dir):
    ...
    plt.colorbar(im)
    plt.tight_layout()
    plt.savefig(os.path.join(output_dir, "results", "svm_confusion_matrix.png"),
...
def main():
    ...
    joblib.dump(scaler, os.path.join(args.output_dir, "checkpoints", "scaler.pkl"))

    le = LabelEncoder()
    y_train = le.fit_transform(y_train_raw)
    y_test = le.transform(y_test_raw)
    joblib.dump(le, os.path.join(args.output_dir, "checkpoints", "label_encoder.pkl"))

    # 5. Train SVM model with GridSearchCV
    print("\nTraining SVM with Hyperparameter Tuning...")
    svm_params = {"C": [0.1, 1, 10], "gamma": ["scale", "auto"], "kernel": ["rbf"]}
    svm_grid = GridSearchCV(SVC(random_state=RANDOM_SEED, probability=True),
    ...
    joblib.dump(svm_best, os.path.join(args.output_dir, "checkpoints", "svm.pkl"))
    print(f"Saved SVM model to {args.output_dir}/checkpoints/svm.pkl")
    ...
    report_path = os.path.join(args.output_dir, "results", "svm_metrics_report.json")
